...
void transformCameraToRobot(geometry_msgs::PointStamped &camera_point,
  ...
  listener.waitForTransform("/camera_link", "/arm_link_0",
    ros::Time(0), ros::Duration(10.0));

  // transform camera_point to arm_link_point
  try
  {
    listener.transformPoint("arm_link_0", camera_point, arm_link_point);
  }
...
void pixelTo3DPointWithPartialCloud(const sensor_msgs::PointCloud2 pCloud,
  ...
  s = 0. + tempMat2.at<double>(2,0); //0. represents the height
  ...
  arm_link_point.header.frame_id = "arm_link_0";
  arm_link_point.header.stamp = ros::Time();
  arm_link_point.point.x = float(tempMat2.at<double>(0,0));
  arm_link_point.point.x = float(tempMat2.at<double>(1,0));
  arm_link_point.point.x = float(tempMat2.at<double>(2,0));
...
void camera_info_cb(const sensor_msgs::CameraInfo input){
  ...
  for (int idx = 0; idx < 5; idx++){
    distCoeffs.at<double>(idx, 0) = input.K[idx];
  }
